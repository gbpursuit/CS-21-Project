...
# ==============================================================
    ...
    up_combine_check_loop:
        ...
        beqz $t5, up_skip_combine     # Skip if current cell is zero
        ...
        # Adjust target position ($t2) after combine
        addi $t2, $t1, 1
    ...
    move_up_clear_loop:
        mul $t6, $t2, 3            # Row offset for clearing
        ...
        addi $t2, $t2, 1           # Move to the next row for clearing
        li $t8, 3
        blt $t2, $t8, move_up_clear_loop
...
# ==============================================================
    ...
    down_combine_check_loop:
        ...
        beqz $t5, down_skip_combine     # Skip if current cell is zero
    ...
    move_down_clear_loop:
        ...
        addi $t2, $t2, -1          # Move to the next row for clearing
        li $t8, -1
        bgt $t2, $t8, move_down_clear_loop
